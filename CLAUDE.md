...
- `core/traffic.py` — команда `/traffic`, скрапит балл пробок Омска с ngs55.ru регуляркой по HTML. Также фоновый `traffic_broadcast_scheduler()` (крутится параллельно с `listen()` через `asyncio.gather()` в `main()`, стартует с задержкой 5 c по аналогии с `advert_scheduler`) шлёт тот же отчёт в `TRAFFIC_CHANNEL_IDX` с интервалом `TRAFFIC_INTERVAL_MINUTES` (не чаще раза в час — значение всегда зажимается снизу до 60; `0` отключает рассылку целиком) и только внутри окна `TRAFFIC_HOUR_FROM`..`TRAFFIC_HOUR_TO` (по умолчанию 7–19 по местному времени) — тик вне окна молча пропускается, следующий сработает через тот же интервал. Рассылка также пропускается, если балл пробок равен 0.
