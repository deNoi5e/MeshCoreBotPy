...
- `core/traffic.py` — команда `/traffic`, скрапит балл пробок Омска с ngs55.ru регуляркой по HTML. Также фоновый `traffic_broadcast_scheduler()` (крутится параллельно с `listen()` через `asyncio.gather()` в `main()`, стартует с задержкой 5 c по аналогии с `advert_scheduler`) с интервалом `TRAFFIC_INTERVAL_MINUTES` (значение зажимается снизу до 5 минут; `0` отключает проверку целиком) опрашивает балл пробок и шлёт отчёт в `TRAFFIC_CHANNEL_IDX`, только если балл изменился по сравнению с прошлой проверкой — это отделяет частоту опроса сайта от частоты сообщений в канал. Последний известный балл хранится не только в памяти (`last_score` внутри цикла шедулера), но и в файле `traffic_last_score.txt` в корне проекта (не в git, см. `.gitignore`) — читается при старте и перезаписывается при каждом обнаруженном изменении, чтобы рестарт бота не воспринимал текущий балл как «изменение с нуля» и не слал его в канал без реальной причины. Отправка в канал дополнительно ограничена окном `TRAFFIC_HOUR_FROM`..`TRAFFIC_HOUR_TO` (по умолчанию 7–19 по местному времени): изменение вне окна всё равно сохраняется (в памяти и в файле), чтобы не разослать его задним числом при следующей проверке, но в канал не уходит.
